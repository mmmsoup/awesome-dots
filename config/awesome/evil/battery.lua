...
awful.spawn.easy_async_with_shell("sh -c 'out=$(find /sys/class/power_supply/ACAD/online) && (echo $out | head -1) || false' ", function (battery_file, _, __, exit_code)
    -- No battery file found
    --if not (exit_code == 0) then
    --    return
    --end
    ...
    awful.widget.watch("cat "..battery_file, update_interval, function(_, stdout)
        if stdout == "1\n" then
            if plugged_state == true then
                awesome.emit_signal("evil::charger", plugged_state)
            end
            plugged_state = true
        else
            if plugged_state == true then
